...
void AltimeterWidget::setFont(const sf::Font* font)
{
	m_font = font;
}


void AltimeterWidget::setAltitudeMeters(float altitudeMeters)
{
	m_altitudeMeters = altitudeMeters;
}


void AltimeterWidget::setPosition(float x, float y)
{
	m_centerX = x;
	m_centerY = y;
}
...
void AltimeterWidget::setRadius(float radius)
{
	m_radius = radius;
}


void AltimeterWidget::draw(sf::RenderTarget& target) const
{
	...
	for(int i = 0; i <= 10; ++i)
	{
		...
		if(isMajor and m_font != nullptr and i < 10)
		{
			int labelValue = i * 100;
			...
			text.setPosition(m_centerX + std::cos(angleRad) * (m_radius - 32.0f),
							 m_centerY + std::sin(angleRad) * (m_radius - 32.0f));
			
			target.draw(text);
		}
	}
	
	float clampedAltitude = m_altitudeMeters;
	if(clampedAltitude < 0.0f)
		clampedAltitude = 0.0f;
	
	float wrappedAltitude = std::fmod(clampedAltitude, 1000.0f);
	float normalized = wrappedAltitude / 1000.0f;
	...
	if(m_font != nullptr)
	{
		...
		valueText.setString(std::to_string(static_cast<int>(m_altitudeMeters)) + " m");
