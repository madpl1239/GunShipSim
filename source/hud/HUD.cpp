...
void HUD::draw(sf::RenderWindow& window)
{
	...
	m_speedWidget.setRadius(88.0f);
	m_speedWidget.draw(window);
	
	m_compassWidget.setHeadingDegrees(m_headingDegrees);
	m_compassWidget.setPosition(310.0f, gaugeCenterY);
	m_compassWidget.setRadius(88.0f);
	...
	m_verticalSpeedWidget.setRadius(88.0f);
	m_verticalSpeedWidget.draw(window);
	
	m_aglWidget.setAltitudeAboveGroundMeters(m_altitudeAboveGroundMeters);
	m_aglWidget.setPosition(static_cast<float>(windowSize.x) - 320.0f, gaugeCenterY);
	m_aglWidget.setRadius(88.0f);
