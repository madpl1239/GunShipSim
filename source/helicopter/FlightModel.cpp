...
void FlightModel::update(float dt, const HelicopterInputState& inputState)
{
	...
	m_targetRollDegrees = -45.0f * inputState.yawInput * (0.35f + speedRatio * 0.65f);
	
	const float pitchResponse = 5.0f;
	m_pitchDegrees += (m_targetPitchDegrees - m_pitchDegrees) * std::min(1.0f, pitchResponse * dt);
	
	const float maxRollSpeedDegreesPerSecond = 8.0f;
	const float maxRollStep = maxRollSpeedDegreesPerSecond * dt;
	const float rollDelta = m_targetRollDegrees - m_rollDegrees;
	
	if(rollDelta > maxRollStep)
		m_rollDegrees += maxRollStep;
	
	else if(rollDelta < -maxRollStep)
		m_rollDegrees -= maxRollStep;
	
	else
		m_rollDegrees = m_targetRollDegrees;
}
